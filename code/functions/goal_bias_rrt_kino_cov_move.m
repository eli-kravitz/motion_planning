function [node,ni,T,time,P] = goal_bias_rrt_kino_cov_move(q_start,q_goal,...
...
while n_iter < n
    ...
    t_tot = 0;
    while parent ~= 0
        idx_new = find(T(:,1) == parent);
        t_tot = t_tot+T(idx_new,end);
        parent = T(idx_new,2);
    end
    t_tot = t_tot+dt;
    
    % Check for validity of trajectory
    traj_valid = validity_check_kino_move(v,pts_new,l_car,w_car,y_lim,...
        v_lim,phi_lim,L,t_tot,vel);
    if traj_valid
